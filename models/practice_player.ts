...
@TypeORM.Entity()
export default class PracticePlayer extends Model {
  static cache = true;
  ...
  static async findInPractice(where) {
    return PracticePlayer.findOne({ where: where });
  }

  async loadRelationships() {
    this.user = await User.findById(this.user_id);
    this.practice = await Practice.findById(this.practice_id);
  }
