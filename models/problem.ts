...
export default class Problem extends Model {
  ...
  async isAllowedUseBy_contest(user) {                            // 判断 该问题 属于一场user可参加的 已结束的比赛
      if (!user) return false;                                    // 未登录, false
      if (user.id === this.user_id) return true;                  // 问题创建者, true
      if (await user.hasPrivilege('manage_problem')) return true; // 有manage权限, true
      if (!this.is_public) return false;                          // 未公开, false

      return ( await user.getcontprobs() ).indexOf(this.id) != -1 ;// 根据比赛获得完全题目列表 判断
  }

  async isAllowedUseBy_practice(user) {                            // 判断 该问题 属于一场user可参加的 已结束的比赛
    if (!user) return false;                                    // 未登录, false
    if (user.id === this.user_id) return true;                  // 问题创建者, true
    if (await user.hasPrivilege('manage_problem')) return true; // 有manage权限, true
    // if (!this.is_public) return false;                          // 未公开, false
    return ( await user.getpracprobs() ).indexOf(this.id) != -1 ;// 根据比赛获得完全题目列表 判断
  }
  ...
  async isAllowedEvaluateBy(user) {
    if (!user) return false;
    let cnt = await JudgeState.count({ problem_id: this.id, user_id: user.id, score: TypeORM.MoreThanOrEqual(15) });
    return cnt != 0;
  }
  ...
  async resetSubmissionCount() {
    ...
    await syzoj.utils.lock(['Problem::resetSubmissionCount', this.id], async () => {
      ...
      let x = await entity.query(`SELECT submit_time from judge_state j WHERE problem_id = ${this.id} AND status = 'Accepted' AND NOT exists(SELECT id FROM judge_state j2 WHERE problem_id = ${this.id} AND j2.user_id=j.user_id AND j2.id < j.id AND status = 'Accepted' LIMIT 0,1) ORDER BY id DESC LIMIT 0,1`)
      // let judge_state = this.ac_num > 0 ? await JudgeState.findOne({where: {problem_id: this.id, status: 'Accepted'}, order: { id: "DESC" }}) : undefined
      this.submit_num =  x.length > 0 ? parseInt(x[0]['submit_time']) : this.id
  ...
  async changeID(id) {
    ...
    for (let practice of practices) {
      let problemIDs = await practice.getProblems();
      ...
      if (flag) {
        await practice.setProblemsNoCheck(problemIDs);
        await practice.save();
      }
    }
