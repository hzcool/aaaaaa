...
@TypeORM.Entity()
export default class PracticeRanklist extends Model {
  ...
  async getPlayers() {
    ...
    for (let i = 1; i <= this.ranklist.player_num; i++) {
      a.push(await PracticePlayer.findById(this.ranklist[i]));
    }
    return a;
  }

  async updatePlayer(practice, player) {
      ...
      for (let player of players) {
        ...
        for (let i in player.score_details) {
          if (player.score_details[i].accepted) {
            ...
            player.score += player.score_details[i].weighted_score;
            player.timeSum += (player.score_details[i].acceptedTime - practice.start_time) + (player.score_details[i].unacceptedCount * 20 * 60);
          }
        }
        await player.save ();
      }
